...
Simulations for the popularity spread model are provided in the subfolder `Popularity_spread/`. This module is run separately from the SIR–Fokker–Planck code and is driven by its own main script, `popularity_main.m`. All relevant settings are collected in the first part of the script, where the user can modify the model and numerical parameters.

The popularity model is not standalone, as it requires as input the output of a previously computed SIR–Fokker–Planck simulation. In particular, `popularity_main.m` loads the time-dependent distribution `ftot_over_time`, the corresponding time grid `time`, and the spatial–opinion grids `x`, `w` produced by `kineticSIR_main.m`.
